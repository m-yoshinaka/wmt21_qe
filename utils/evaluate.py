...
There are some modifications from original script.
"""

language_pairs = ['en-de', 'en-zh', 'ro-en',
                  'et-en', 'ne-en', 'si-en', 'ru-en']
...
def parse_submission(pred_list):
    ...
    for line in pred_list:
        ...
        assert len(pred) == 4, \
            "Incorrect format, expecting (tab separated): <LP> <METHOD_NAME> <SEGMENT_NUMBER> <SEGMENT_SCORE>."  # noqa:E501
    ...
    assert set(language_pairs) == set(lp_dict_keys), \
        "Incorrect list of LPs, expecting: {}, given: {}.".format(
            language_pairs, lp_dict_keys)
    ...
    for lp_str in lp_dict_keys:
        ...
        lp_dict[lp_str] = np.array(
            list(dict(sorted(tmp_lp_segments.items())).values()))
...
if __name__ == '__main__':
    ...
    for lp_str in predictions:
        ...
        assert len(predictions[lp_str]) == len(goldlabels[lp_str]), \
            "Incorrect number of predicted scores for {}, expecting {}, given {}.".format(  # noqa:E501
